...
function tvChart(sym){const box=document.getElementById('tvchart');box.innerHTML='<div class="tradingview-widget-container" style="width:100%;height:100%"><div class="tradingview-widget-container__widget" style="width:100%;height:100%"></div></div>';const host=box.firstElementChild;const s=document.createElement('script');s.src='https://s3.tradingview.com/external-embedding/embed-widget-advanced-chart.js';s.type='text/javascript';s.async=true;s.innerHTML=JSON.stringify({autosize:true,symbol:'NASDAQ:'+sym,interval:'D',timezone:'exchange',theme:'light',style:'1',withdateranges:true,hide_side_toolbar:false,allow_symbol_change:true,save_image:false,calendar:false,support_host:'https://www.tradingview.com',studies:['Volume@tv-basicstudies']});host.appendChild(s)}
...
function navTo(target){const el=document.getElementById(target);if(!el)return;el.scrollIntoView({behavior:'smooth',block:'start'});el.style.outline='2px solid #315fd6';setTimeout(()=>el.style.outline='',1200)}
...
function setupNavigation(){const map={Stocks:'portfolioPanel',Explore:'explorePanel',Holdings:'holdingsPanel',Positions:'positionsPanel',Orders:'ordersPanel',Watchlist:'scannerPanel',Scanner:'scannerPanel',Strategy:'strategyPanel',Settings:'settingsPanel'};document.querySelectorAll('.nav button').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('.nav button').forEach(x=>x.classList.remove('active'));b.classList.add('active');navTo(map[b.textContent.trim()]||'portfolioPanel')}));document.querySelectorAll('.bottom span').forEach(b=>b.addEventListener('click',()=>{const t=b.innerText.split('\n')[1]||'';const m={Stocks:'portfolioPanel',Markets:'explorePanel',Trade:'tvchart',Portfolio:'holdingsPanel',Orders:'ordersPanel',More:'settingsPanel'};navTo(m[t]||'portfolioPanel')}));document.getElementById('saveApi').addEventListener('click',saveApi)}
