...
function tvMarketTicker(){const box=document.getElementById('marketTicker');box.innerHTML='<div class="tradingview-widget-container" style="width:100%;height:64px"><div class="tradingview-widget-container__widget"></div></div>';const s=document.createElement('script');s.src='https://s3.tradingview.com/external-embedding/embed-widget-ticker-tape.js';s.async=true;s.innerHTML=JSON.stringify({symbols:[{description:'NIFTY 50',proName:'NSE:NIFTY'},{description:'SENSEX',proName:'BSE:SENSEX'},{description:'NASDAQ',proName:'NASDAQ:IXIC'},{description:'S&P 500',proName:'SP:SPX'}],showSymbolLogo:false,colorTheme:'light',isTransparent:false,displayMode:'adaptive',locale:'en'});box.firstChild.appendChild(s)}
function navTo(target){const el=document.getElementById(target);if(el)el.scrollIntoView({behavior:'smooth',block:'start'})}
function setupNavigation(){const map={Stocks:'portfolioPanel',Explore:'explorePanel',Holdings:'holdingsPanel',Positions:'positionsPanel',Orders:'ordersPanel',Watchlist:'scannerPanel',Scanner:'scannerPanel',Strategy:'strategyPanel'};document.querySelectorAll('.nav button').forEach(b=>b.addEventListener('click',()=>navTo(map[b.textContent.trim()]||'portfolioPanel')));document.querySelectorAll('.bottom span').forEach(b=>b.addEventListener('click',()=>{const t=b.innerText.split('\n')[1]||'';const m={Stocks:'portfolioPanel',Markets:'explorePanel',Trade:'chart',Portfolio:'portfolioPanel',Orders:'ordersPanel',More:'scannerPanel'};navTo(m[t]||'portfolioPanel')}))}
