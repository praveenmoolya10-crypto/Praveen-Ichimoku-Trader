...
<div class="ticker"><div class="item"><b>NIFTY 50</b><strong>—</strong><small class="hint">NSE</small></div><div class="item"><b>SENSEX</b><strong>—</strong><small class="hint">NSE</small></div><div class="item"><b>NASDAQ</b><strong>—</strong><small class="hint">Alpaca</small></div><div class="item"><b>S&P 500</b><strong>—</strong><small class="hint">Alpaca</small></div></div>
<div class="nav"><button class="active">Stocks</button><button>Explore</button><button>Holdings</button><button>Positions</button><button>Orders</button><button>Watchlist</button><button>Scanner</button><button>Strategy</button></div>
<div class="content">
...
<div class="card"><div class="chartHead"><div><div id="symbol" class="symbol">AAPL</div><div id="last" class="hint">Loading market data...</div></div><div id="signal" class="signal">HOLD</div></div><div class="chartBox"><canvas id="chart"></canvas></div><div class="legend"><span>▮ Price</span><span>— Tenkan</span><span>— Kijun</span><span>Cloud = Senkou A/B</span></div></div>
<div class="card"><div class="stats"><div class="stat"><small>Open</small><b id="open">—</b></div><div class="stat"><small>High</small><b id="high">—</b></div><div class="stat"><small>Low</small><b id="low">—</b></div><div class="stat"><small>Volume</small><b id="volume">—</b></div></div></div>
<div class="grid"><div class="card"><div class="chartHead"><b>Portfolio</b><span class="pill">Paper</span></div><div class="row"><span>Account value</span><b id="value">—</b></div><div class="row"><span>Cash</span><b id="cash">—</b></div><div class="row"><span>Buying power</span><b id="bp">—</b></div></div><div class="card"><div class="chartHead"><b>Strategy</b><span id="configured" class="hint">Not configured</span></div><div class="row"><span>Ichimoku</span><b class="green">ACTIVE</b></div><div class="row"><span>Auto trading</span><b id="auto">OFF</b></div><div class="row"><span>Emergency stop</span><b class="red">ON</b></div></div></div>
<div class="grid"><div class="card"><div class="chartHead"><b>Positions</b><span class="hint">Live</span></div><div id="positions" class="watch"></div></div><div class="card"><div class="chartHead"><b>Orders</b><span class="hint">Live</span></div><div id="orders" class="watch"></div></div></div>
<div class="card"><div class="chartHead"><b>Ichimoku Scanner</b><span class="hint">10 stocks</span></div><div id="scanner" class="watch"></div></div>
...
<script>
const syms=["AAPL","MSFT","NVDA","AMZN","GOOGL","META","TSLA","NFLX","AVGO","AMD"];let bars=[];let ind=null;
async function get(u){try{let r=await fetch(u);return await r.json()}catch(e){return {}}}
function draw(){const c=document.getElementById('chart'),box=c.parentElement,dpr=devicePixelRatio||1,w=box.clientWidth,h=box.clientHeight;c.width=w*dpr;c.height=h*dpr;let x=c.getContext('2d');x.scale(dpr,dpr);x.clearRect(0,0,w,h);if(!bars.length){x.fillStyle='#8792a2';x.font='14px Arial';x.fillText('Market data unavailable — connect Alpaca',20,30);return}let n=Math.min(80,bars.length),a=bars.slice(-n),lo=Math.min(...a.map(v=>+v.l)),hi=Math.max(...a.map(v=>+v.h)),pad={l:8,r:8,t:20,b:28};let X=i=>pad.l+i*(w-pad.l-pad.r)/(n-1),Y=v=>pad.t+(hi-v)*(h-pad.t-pad.b)/(hi-lo||1);x.strokeStyle='#e9edf3';x.lineWidth=1;for(let i=0;i<5;i++){let y=pad.t+i*(h-pad.t-pad.b)/4;x.beginPath();x.moveTo(pad.l,y);x.lineTo(w-pad.r,y);x.stroke()}for(let i=0;i<n;i++){let v=a[i],xx=X(i),oh=Y(+v.o),hh=Y(+v.h),ll=Y(+v.l),cc=Y(+v.c),up=+v.c>=+v.o;x.strokeStyle=up?'#16a67a':'#e25555';x.fillStyle=up?'#16a67a':'#e25555';x.beginPath();x.moveTo(xx,hh);x.lineTo(xx,ll);x.stroke();let bw=Math.max(2,(w-pad.l-pad.r)/n*.62);x.fillRect(xx-bw/2,Math.min(oh,cc),bw,Math.max(1,Math.abs(cc-oh)))}x.fillStyle='#7d8795';x.font='11px Arial';for(let i=0;i<5;i++){let val=hi-(hi-lo)*i/4;x.fillText(val.toFixed(2),8,pad.t+i*(h-pad.t-pad.b)/4-3)}}
async function load(){let s=document.getElementById('sym').value,tf=document.getElementById('tf').value;let [b,st,a,p,o]=await Promise.all([get('/api/bars/'+s+'?timeframe='+tf+'&limit=200'),get('/api/strategy/'+s),get('/api/account'),get('/api/positions'),get('/api/orders')]);bars=b.bars?.[s]||[];ind=st;document.getElementById('symbol').textContent=s;document.getElementById('signal').textContent=st.signal||'HOLD';let z=bars.at(-1);if(z){document.getElementById('last').textContent='LTP '+Number(z.c).toFixed(2)+' • '+new Date(z.t).toLocaleString();document.getElementById('open').textContent=Number(z.o).toFixed(2);document.getElementById('high').textContent=Number(z.h).toFixed(2);document.getElementById('low').textContent=Number(z.l).toFixed(2);document.getElementById('volume').textContent=Number(z.v).toLocaleString()}else document.getElementById('last').textContent='Market data unavailable';document.getElementById('value').textContent=a.equity?'$'+a.equity:'—';document.getElementById('cash').textContent=a.cash?'$'+a.cash:'—';document.getElementById('bp').textContent=a.buying_power?'$'+a.buying_power:'—';document.getElementById('configured').textContent=a.status==='not_configured'?'Not configured':'Connected';document.getElementById('auto').textContent=a.status==='not_configured'?'OFF':'OFF';document.getElementById('positions').innerHTML=(p||[]).map(q=>'<div class="row"><b>'+q.symbol+'</b><span>'+q.qty+' @ '+q.avg_entry_price+'</span></div>').join('')||'<div class="hint">No open positions</div>';document.getElementById('orders').innerHTML=(o||[]).slice(0,8).map(q=>'<div class="row"><b>'+q.symbol+' '+q.side+'</b><span>'+q.status+'</span></div>').join('')||'<div class="hint">No orders</div>';let ss=await Promise.all(syms.map(async q=>[q,await get('/api/strategy/'+q)]));document.getElementById('scanner').innerHTML=ss.map(q=>'<div class="row"><b>'+q[0]+'</b><span class="'+(q[1].signal==='BUY'?'green':q[1].signal==='SELL'?'red':'hint')+'">'+(q[1].signal||'HOLD')+'</span></div>').join('');draw()}
document.getElementById('sym').addEventListener('change',load);document.getElementById('tf').addEventListener('change',load);document.getElementById('refresh').addEventListener('click',load);document.getElementById('search').addEventListener('change',e=>{let v=e.target.value.trim().toUpperCase();if(syms.includes(v)){document.getElementById('sym').value=v;load()}});window.addEventListener('resize',draw);load();setInterval(load,30000);
