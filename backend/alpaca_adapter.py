...
class AlpacaAdapter:
    ...
    def orders(self):
        if not self.client:return []
        return [o.model_dump() for o in self.client.get_orders()]
    def bars(self,symbol,timeframe,limit):
        ...
        req=StockBarsRequest(symbol_or_symbols=symbol.upper(),timeframe=tf,start=datetime.now(timezone.utc)-timedelta(days=max(limit*2,30)),limit=limit)
        data=self.data.get_stock_bars(req)
        raw=data.data.get(symbol.upper(),[])
        return {"bars":{symbol.upper():[{"t":x.timestamp.isoformat(),"o":x.open,"h":x.high,"l":x.low,"c":x.close,"v":x.volume} for x in raw]}}
    def place_order(self,symbol,side,qty,order_type,tif):
        if not self.client: raise RuntimeError("Alpaca is not configured")
        req=MarketOrderRequest(symbol=symbol.upper(),qty=qty,side=OrderSide.BUY if side.lower()=="buy" else OrderSide.SELL,time_in_force=TimeInForce.DAY)
